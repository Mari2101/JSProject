...
let id: number;
let chooseAnim = document.getElementById("choose-p") as HTMLParagraphElement;
let randomChoose = document.getElementById("random-btn") as HTMLButtonElement;
...
randomChoose.addEventListener("click", () => {
  ...
  setTimeout(() => {
    ...
    id = setInterval(() => {
      ...
      if (
        img.getBoundingClientRect().x + chosenAnimal.step * 5 >
...
animalArray
  ...
  .map((animal) => {
    ...
    image.addEventListener("click", () => {
      ...
      const chosenAnimalImg = document.getElementById(
        chosenAnimal.id
      ) as HTMLImageElement;
      ...
      animalArray.forEach((animal) => {
        let animalImage = document.getElementById(
          animal.id
        ) as HTMLImageElement;
        setTimeout(() => {
          const audio = new Audio(`./media/${chosenAnimal.voice}.wav`);
          ...
          let id = setInterval(() => {
            ...
            if (
              animalImage.getBoundingClientRect().x + animal.step * 11 >=
              document.body.getBoundingClientRect().width
            ) {
              animal.translateX =
              ...
              setTimeout(() => {
                let winnerP = document.getElementById(
                  "winner"
